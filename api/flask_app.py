...
huggingface_repo_id = "mistralai/Mistral-7B-Instruct-v0.2"  # Default model
...
def load_llm_model(repo_id=huggingface_repo_id, api_token=TOKEN):
    """Loads the Hugging Face Endpoint LLM."""
    global llm
    print(f"Attempting to load LLM: {repo_id}")
    ...
    try:
        ...
        print(f"FATAL: Error loading LLM '{repo_id}': {e}\n{traceback.format_exc()}")
        return None
...
async def startup_event():  # async can works like that if the upper line of code take time to execute they cant effect on them they start processing this async function in the same time
    ...
    llm = load_llm_model(huggingface_repo_id, TOKEN)
    if llm is None:
